...
/*
struct dmz_cgrp_mapping {
	struct list_head	list;
	int 			css_id;
	bool 			*mapped;
	unsigned short		*cg_chunk;
}
*/
...
struct dmz_metadata {
	...
	/* Chunk mapping info per Cgroup */
	//struct dmz_cgrp_mapping	*cg_map_head;
	//struct radix_tree_root	cg_map_rxtree;
};
...
void dmz_report_zones(struct dmz_metadata *zmd, unsigned int chunk){
	struct dmz_mblock *dmap_mblk = zmd->map_mblk[chunk >> DMZ_MAP_ENTRIES_SHIFT];
	struct dmz_map *dmap = (struct dmz_map *) dmap_mblk->data;
	...
	if (dzone_id == DMZ_MAP_UNMAPPED) {
		printk("%s: chunk %u has no mapping to zone",__func__, chunk);
	}
	else {
		zone = dmz_get(zmd, dzone_id);
		printk("%s: Report Zone %u is mapped to chunk %u valid block %u",
				__func__,
				zone->id,
				chunk,
				zone->weight);
...
unsigned int dmz_check_chunk_mapping(struct dmz_metadata *zmd, unsigned int chunk)
{
	struct dmz_mblock *dmap_mblk = zmd->map_mblk[chunk >> DMZ_MAP_ENTRIES_SHIFT];
	struct dmz_map *dmap = (struct dmz_map *) dmap_mblk->data;
	int dmap_idx = chunk & DMZ_MAP_ENTRIES_MASK;
	...
	if (dzone_id == DMZ_MAP_UNMAPPED) return 0;
	else return 1;
	/*
	{
		dzone = dmz_get(zmd, dzone_id);
...
void dmz_put_chunk_mapping(struct dmz_metadata *zmd, struct dm_zone *dzone)
{
	...
	//printk("%s: zone %u is unmapped",__func__,);
	dmz_unlock_map(zmd);
}
		     ...
		     const char *devname)
{
	...
	*/
	...
	/*
	cg_map_head = kmalloc(sizeof(struct dmz_cgrp_mapping), GFP_NOIO);
	zmd->cg_map_head = cg_map_head;
	zmd->cg_map_head-> css_id = 0;
	INIT_LIST_HEAD(&zmd->cg_map_head->list);

	INIT_RADIX_TREE(&zmd->cg_map_rxtree, GFP_NOIO);
	*/
